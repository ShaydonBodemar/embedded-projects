...
void ColorsClock(void){
	...
	case RED_DN:
		...
		if (RedPwmSetting < RGB_RED_PWM_MIN){
			...
			RgbColorsState = GRN_UP;
		}
		break;

	case GRN_UP:
		RgbGrnPwmSet(GreenPwmSetting++);
		if (GreenPwmSetting > RGB_GRN_PWM_MAX){
			GreenPwmSetting = RGB_GRN_PWM_MAX;
			RgbColorsState = GRN_DN;
		}
		break;

	case GRN_DN:
		RgbGrnPwmSet(GreenPwmSetting--);
		if (GreenPwmSetting < RGB_GRN_PWM_MIN){
			GreenPwmSetting = RGB_GRN_PWM_MIN;
			RgbColorsState = BLU_UP;
		}
		break;

	case BLU_UP:
		RgbBluPwmSet(BluePwmSetting++);
		if (BluePwmSetting > RGB_BLU_PWM_MAX){
			BluePwmSetting = RGB_BLU_PWM_MAX;
			RgbColorsState = BLU_DN;
		}
		break;

	case BLU_DN:
		RgbBluPwmSet(BluePwmSetting--);
		if (BluePwmSetting < RGB_BLU_PWM_MIN){
			BluePwmSetting = RGB_BLU_PWM_MIN;
			RgbColorsState = RED_GRN_UP;
		}
		break;
